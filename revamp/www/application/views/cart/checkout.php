...
<div id="checkout_content">
	...
	<tbody>
		<? foreach ($cartItems as $idx=>$cartItem) {?>
			<tr>
				<td>
					<? echo HTML::anchor("/subcategory/product_detail/".$cartItem->product_id, HTML::image(PRODUCT_IMAGE_PATH.$cartItem->product_id.'_s.jpg',array('width'=>'250')).'<br>'.$cartItem->getFullProductName()); ?>
				</td>
				<td><?=$cartItem->name ?></td>
...
<? echo Form::open("cart/confirm", array('id'=>'form1')); ?>
	...
	<div id="checkout_step2">
		...
		<tbody>
			...
			<tr>
				<td colspan="2">
					<iframe name="termofpayment" id="termofpayment" src="../term/term_<?=LANGUAGE ?>.htm" style="width:100%"></iframe>
					<div id="is_agree">
						<? echo Form::checkbox('is_agree', 'Y', $form->is_agree == 'Y'); ?>&nbsp;<? echo __('checkout.checkbox.agree_term'); ?>
					</div>
				</td>
			</tr>
	...
	jq(function() {
		<? if(isset($errors)) {
			foreach($errors as $error_key=>$error) {?>
				...
				jq("div[id='<?=$error_key ?>'][id!='address']").addClass('error_input');
			<? }
		}?>
